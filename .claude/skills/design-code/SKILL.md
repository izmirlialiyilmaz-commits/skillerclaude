...
13. **Run the gates, then report (mandatory).** Before declaring done, RUN the bundled gates over YOUR output and report their actual stdout: `python3 scripts/lint_hardcodes.py <path>`, `python3 scripts/validate_theme_refs.py <theme.css> <src>`, `python3 scripts/lint_taste.py <files>`, and for rendered HTML `node scripts/verify_states.mjs <file>` + `--dark` (every element, default/hover/focus). Never type a contrast number or "100%" you didn't just measure (see the `verification-before-completion` skill).

## Bundled files (this skill is self-contained)

| Path | What |
|:---|:---|
| `frameworks/` | Adapter protocol, full references (React+Tailwind, Next.js, SwiftUI) and 16 concise adapters |
| `components/` | Component specs (atoms → organisms, overlays, data-viz, icon system) |
| `accessibility/aria-patterns.md` | ARIA pattern reference |
| `tokens/` | The 3-tier DTCG token set (`colors`, `typography`, `spacing`, `motion`, …) |
| `taste/design-taste.md` | The 12-point aesthetic pre-flight |
| `content/voice-tone.md` | UI copy voice + destructive-confirmation wording |
| `workflows/redesign-audit.md` | Output-completeness rules |
| `rules/tokens-and-color.md` | Single-theme consistency rules |
| `examples/golden/` | Reference `Button.tsx`, `Modal.tsx`, `theme.css` |
| `examples/sample-app/preview.html` | Destructive-confirmation UX example |
| `scripts/` | The gates — see below |

### Gates and their prerequisites

- `contrast.py`, `lint_hardcodes.py`, `lint_taste.py`, `validate_theme_refs.py` — `python3`
  only, no dependencies. Each takes the paths you want checked; run `--help`-style usage is
  in each file's docstring.
- `taste_audit.mjs`, `verify_states.mjs` — render-based, so they need Node **and**
...
The upstream `accuracy_report.mjs` gate and its `npm run verify` wrapper are that repo's own
CI self-test over its own example tree — they do not test your generated code, so they are
deliberately left out. The sibling skills this file mentions (`apply-aesthetic`,
`a11y-audit`, `design-tokens`) are not installed here; do their work inline instead.
