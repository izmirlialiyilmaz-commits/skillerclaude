...
2. Run `bash .claude/skills/watch/watch.sh <args>` (i.e. `watch.sh` inside this skill directory). It will:
...
4. Apply the prompt for the mode — `prompts/hook-analysis.md`, `prompts/retention-review.md`, or `prompts/library-ingest.md` — and produce the report.
...
6. **Always render an HTML report and open it in the browser. This is mandatory on every `/watch` run, not a follow-up step.**
   ...
   - Then open it in the user's default browser via Bash: `open <workdir>/report.html` on macOS, `xdg-open <workdir>/report.html` on Linux. If neither exists (headless/remote session), skip the launch and print the path instead.
