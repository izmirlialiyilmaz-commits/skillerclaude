...
case "$MODE" in
  hook)      PROMPT_FILE="hook-analysis.md" ;;
  retention) PROMPT_FILE="retention-review.md" ;;
  library)   PROMPT_FILE="library-ingest.md" ;;
  *) echo "--mode must be hook|retention|library (got: $MODE)" >&2; exit 1 ;;
esac
...
# Load .env so API keys / price overrides are available.
# Looked for next to this script first, then one level up (repo-style layout).
for ENV_FILE in "$SCRIPT_DIR/.env" "$(cd -- "$SCRIPT_DIR/.." &>/dev/null && pwd)/.env"; do
...
PROMPT: $SCRIPT_DIR/prompts/${PROMPT_FILE}
COST: $WORK/cost.json
